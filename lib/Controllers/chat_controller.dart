...
void startListening() async {
  ...
  // Start the no-speech timer to auto-stop if no speech detected 
  startNoSpeechTimer();
  isListening.value = false;
}

void stopListening() async {
  await speechToText.stop();  // Stop listening
  isListening.value = false;
...
void startNoSpeechTimer() {
  ...
  noSpeechTimer = Timer(const Duration(seconds: 4), () {
...
void onSpeechResult(SpeechRecognitionResult result) {
  ...
  startNoSpeechTimer(); // Restart the no-speech timer on each result

  if (result.finalResult) {
    // If no words are spoken, cancel the timer and stop listening
    if (lastWords.value.isEmpty) {
      isListening.value = false;
    ...
    noSpeechTimer?.cancel(); // Cancel the timer when the final result is received

    // Edit the prompt before making the API call
    editPrompt(2, 3); // You can replace '2' and '3' with dynamic values

    // Call the Gemini API with the speech input and the modified prompt
    _apiService.getChatCompletion(
      model: 'gemini-flash',
      prompt: lastWords.value,
      basePrompt: basePrompt,  // Use the updated basePrompt
    );
  }
}
}